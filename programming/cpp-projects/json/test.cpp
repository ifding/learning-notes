/*
 * Define JSON_TEST_CUSTOM_CONFIG to 1 if you want to build this tester into
...
#endif
#endif // JSON_TEST_CUSTOM_CONFIG

/*
 * Enable or disable code which demonstrates the behavior change in Xcode 7 / Clang 3.7,
 * introduced by DR1467 and described here: https://github.com/dropbox/json/issues/86
 ...
 */
#ifndef JSON_ENABLE_DR1467_CANARY
#define JSON_ENABLE_DR1467_CANARY 0
#endif
...
JSON_TEST_CASE(json_test) {
    ...
    JSON_TEST_ASSERT(obj.dump() == "{\"k1\": \"v1\", \"k2\": 42, \"k3\": [\"a\", 123, true, false, null]}");
    ...
    if (JSON_ENABLE_DR1467_CANARY) {
        Json nested_array = Json::array { Json::array { 1, 2, 3 } };
        JSON_TEST_ASSERT(nested_array.is_array());
        JSON_TEST_ASSERT(nested_array.array_items().size() == 1);
        JSON_TEST_ASSERT(nested_array.array_items()[0].is_array());
        JSON_TEST_ASSERT(nested_array.array_items()[0].array_items().size() == 3);
    }
    ...
    JSON_TEST_ASSERT(((Json)(Json::object { { "foo", nullptr } })).has_shape({ { "foo", Json::NUL } }, err) == true);
    JSON_TEST_ASSERT(((Json)(Json::object { { "foo", 1234567 } })).has_shape({ { "foo", Json::NUL } }, err) == false);
    JSON_TEST_ASSERT(((Json)(Json::object { { "bar", 1234567 } })).has_shape({ { "foo", Json::NUL } }, err) == false);

}

#if JSON_TEST_STANDALONE_MAIN
...
#endif // JSON_TEST_STANDALONE_MAIN

// Insert user-defined suffix code (function definitions, etc)
// to set up a custom test suite
JSON_TEST_CPP_SUFFIX_CODE
